...
- $r < d < u$. In this case, an investment on the stock brings more returns than the risk-free assets like the U.S. treasury bonds. In particular, starting with zero wealth, we can borrow money ($S_0$ amount) from the bank at time $t = 0$ and immediately invest it to the stock, as our worst case wealth from this investment ($S_0 (1 + d)$) is larger than the debt ($S_0 (1 + r)$) we need to pay back to the bank at time $t = 1$, we can pocket the difference and make risk-free profit.  
